...
export class HardhatEthersProvider implements ethers.Provider {
  ...
  public async send(method: string, params?: any[]): Promise<any> {
    ...
    const response = await fetch(this._providerUrl, {
      method: "POST",
      headers: {
  ...
  public async getFeeData(): Promise<ethers.FeeData> {
    ...
    try {
      gasPrice = BigInt(await this.send("eth_gasPrice"));
    } catch {}
    ...
    if (baseFeePerGas !== undefined && baseFeePerGas !== null) {
      try {
        maxPriorityFeePerGas = BigInt(
          await this.send("eth_maxPriorityFeePerGas")
        );
      } catch {
  ...
  public async getBalance(
    ...
    const balance = await this.send("eth_getBalance", [
      resolvedAddress,
      rpcBlockTag,
    ]);
  ...
  public async getTransactionCount(
    ...
    const transactionCount = await this.send(
      "eth_getTransactionCount",
  ...
  public async estimateGas(tx: TransactionRequest): Promise<bigint> {
    ...
    const gasEstimation = await this.send("eth_estimateGas", [
      rpcTransaction,
      rpcBlockTag,
    ]);
  ...
  public async broadcastTransaction(
    ...
    const hashPromise = this.send("eth_sendRawTransaction", [
      signedTx,
    ]);
  ...
  public async getTransactionReceipt(
    ...
    return this._wrapTransactionReceipt(txReceipt);
  }
  ...
  private async _getBlock(
    ...
    if (isHexString(block, 32)) {
      return this.send("eth_getBlockByHash", [
        block,
        includeTransactions,
      ]);
    }
  ...
  private async _isHardhatNetwork(): Promise<boolean> {
    if (this._isHardhatNetworkCached === undefined) {
      ...
      try {
        await this.send("hardhat_metadata");
